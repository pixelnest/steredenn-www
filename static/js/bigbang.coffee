...
# Returns nothing.
doBigBang = (context) ->
  paintFirstLayer(context, 0.2)
  paintFirstLayer(context, 0.5)
  paintSecondLayer(context)
  paintThirdLayer(context)
  paintFourthLayer(context)

paintFirstLayer = (context, alpha) ->
  ...
  COLOR_TABLE = {
    1: "rgba(255, 251, 145, #{alpha})"
    9: "rgba(255, 172, 143, #{alpha})"
    20: "rgba(255, 255, 255, #{alpha})"
    70: "rgba(201, 201, 201, #{alpha})"
  }
  ...
  for i in [0..width]
    for j in [0..height]
      ...
      if canCreateStar()
        ...
        grd = context.createRadialGradient(i, j, 0, i, j, 7)
        grd.addColorStop(0, convertColor(color, 0.2))
        grd.addColorStop(1, 'rgba(23, 23, 23, 0)')

        context.fillStyle = grd
        context.fillRect(i - 8, j - 8, Math.PI*7, Math.PI*7)

        # Get the color and size.
        context.fillStyle = color
        s = 3

        context.fillRect(i-1.5, j-1.5, s, s)
